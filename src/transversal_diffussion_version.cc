...
void Network::calculate_concentrations_b_diff() {
	...
	cerr
			<< "Calculating concentrations for species B taking into account diffusion..."
			<< endl;
	...
	for (int i = 0; i < NN; i++) {
		...
		if (n[i]->t == 1)
			...
			for (int s = 0; s < n[i]->b; s++) {	//eqs for normal and outlet nodes
				...
				if (n[i]->t == -1) {
					...
					S_q += outlet_c_b_0_d(pp, _sign(qq)) - fabs(qq);
				} else {
					B[r_no++] = outlet_c_b_1_d(pp, _sign(qq));
		...
		if (S_q == 0)
			B[r_no] = 1;		//nothing is flowing into the pore
		r_no++;
	}
	...
	if (M_out != 0)
		cerr << "Problem with solving linear equations; M_out = " << M_out
				<< endl;

	cerr << "Filling solution..." << endl;
	for (int i = 0; i < NN; i++)
		n[i]->cb = y[i];     //filling the solution

	//additional printing for debugging
	print_network_for_debugging("After calculating concentration B field ",
...
void Network::calculate_concentrations_c_diff() {
	...
	cerr
			<< "Calculating concentrations for species C taking into account diffusion..."
			<< endl;
	...
	for (int i = 0; i < NN; i++) {
		...
		//cerr<<"W wypisywaniu rownan n = "<<n[i]->a<<endl;
		for (int s = 0; s < n[i]->b; s++) {	//eqs for normal and outlet nodes
			...
			if (nn->t == -1) {       //for  outlet pores
				...
				S_q += outlet_c_c_0_d(pp, _sign(qq)) - qq; //or -fabs(qq), think it over
...
double Network::outlet_c_b_1_d(Pore *p, int s) {
	...
	if (fabs(p->q) > 1e-5 && Pe > 0.001 && Da != -1) { //normal flow through the pore
		...
		double b = s * pe / 2.;
		if (if_track_grains && !(p->is_Va_left()))
			a = s*pe; //no reaction due tu lack of Va
		...
		return -s * fabs(p->q) * (a * exp(a / 2. + b)) / (2. * b * (1 - exp(a)));
			...
			//return 0;
		if (if_track_grains && !(p->is_Va_left()))
			return M_PI * pow(p->d, 2) * D1 / 4 / p->l;

		double dape = DaPe * (d0 / p->d) * pow(p->l / l0, 2);
		double g = p->local_G(this);
		return M_PI * p->d * (k1 / (1 + g)) * p->l / sinh(sqrt(dape))
				/ sqrt(dape);		//dwa roznowazne wzory
...
double Network::outlet_c_b_0_d(Pore *p, int s) {
	...
	if (fabs(p->q) > 1e-5 && Pe > 0.001 && Da != -1) { //normal flow through the pore
		...
		double b = s * pe / 2.;
		if (if_track_grains && !(p->is_Va_left()))
			a = s*pe; //no reaction due to the lack of Va

		return s * fabs(p->q) * (a + 2 * b + (a - 2 * b) * exp(a))
				/ (4. * b * (1 - exp(a)));
		//return  s*p->q*(a + 2*b + (a - 2*b)*exp(a)) / (4.*b*(1-exp(a)));
	}
		...
		double dape = DaPe * (d0 / p->d) * pow(p->l / l0, 2);
		double g = p->local_G(this);
		return - M_PI * p->d * (k1 / (1 + g)) * p->l / tanh(sqrt(dape))
				/ sqrt(dape);		//dwa roznowazne wzory
...
double Network::outlet_c_c_1_d(Pore *p, int s) {
	...
	if (fabs(p->q) > 1e-5 && Pe > 0.001 && Da != -1) { //normal flow through the pore
		...
		double b = s * pe / 2.;
		if (p->d <= d_min)
			a = s*pe; //no reaction due to the lack of space

		//return s*fabs(p->q)*exp(b)*a/(sinh(a/2.)*4.*b); //may be minus in form of the formula
		double tmp = exp(b)/sinh(a/2);
		//cerr<<"tmp 1  = "<<tmp<<endl;
		//cerr<<"all 1 = "<< s * fabs(p->q) * a/(4*b) * tmp<<endl;
		return s * fabs(p->q) * a/(4*b) * tmp; //may be minus in form of the formula
	}
...
double Network::outlet_c_c_0_d(Pore *p, int s) {
	...
	if (fabs(p->q) > 1e-5 && Pe > 0.001 && Da != -1) { //normal flow through the pore
		...
		double tmp = 1./(tanh(a/2.));
		//cerr<<"tmp 0  = "<<tmp<<endl;
		//cerr<<"all 0 = "<< s * fabs(p->q) * (0.5 - a/(4*b)*tmp)<<endl;
		return s * fabs(p->q) * (0.5 - a/(4*b)*tmp);
	}
...
double Network::outlet_c_c_2_d(Pore *p, int s) {
	...
	if (fabs(p->q) > 1e-5 && Pe > 0.001 && Da != -1) { //normal flow through the pore
		...
		double tmp =  + 1./tanh(a/2.)-exp(b)/sinh(a/2.);
		//cerr<<"tmp 2  = "<<tmp<<endl;
		//cerr<<"all 2 = "<< s * fabs(p->q) * R * (0.5 + a/(4*b)*tmp)<<endl;
		return s * fabs(p->q) * R * (0.5 + a/(4*b)*tmp);
	}
